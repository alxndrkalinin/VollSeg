...
from csbdeep.config import BaseConfig
from csbdeep.utils import _raise, load_json, save_json, axes_check_and_normalize, axes_dict, move_image_axes
from csbdeep.utils.six import Path, FileNotFoundError
from csbdeep.data import Normalizer, NoNormalizer
from csbdeep.data import Resizer, NoResizer
...
class BaseModel(object):
    ...
    def __init__(self, config, name=None, basedir="."):
        ...
        config is None or isinstance(config, self._config_class) or _raise(
            ValueError(
                "Invalid configuration of type '%s', was expecting type '%s'."
    ...
    def __repr__(self):
        s = (
            "{self.__class__.__name__}({self.name}): {self.config.axes} → {self._axes_out}\n".format(self=self)
    ...
    def _checkpoint_callbacks(self):
        ...
        if self.basedir is not None:
            ...
            if self.config.train_checkpoint is not None:
                callbacks.append(
                    ModelCheckpoint(
                        str(self.logdir / self.config.train_checkpoint), save_best_only=True, save_weights_only=True
                    )
                )
