...
class OptimizeThreshold(object):
    ...
    def Optimize(self):
        ...
        opt_prob_thresh_voll, opt_measure_voll, opt_nms_thresh, opt_prob_thresh_star, opt_measure_star = (
            None,
            -np.inf,
            None,
            None,
            ...
            def fn(thr):
                ...
                if value_voll is None:
                    ...
                    stats_voll = matching_dataset(
                        Y, Sizedsmart_seeds, thresh=self.iou_threshs, show_progress=False, parallel=True
                    )
                    values_voll[prob_thresh] = value_voll = np.mean([s._asdict()[measure] for s in stats_voll])

                    stats_star = matching_dataset(
                        Y, star_labels, thresh=self.iou_threshs, show_progress=False, parallel=True
                    )
                    values_star[prob_thresh] = value_star = np.mean([s._asdict()[measure] for s in stats_star])

                if verbose > 1:
                    print(
                        "VollSeg, {now}   thresh: {prob_thresh:f}   {measure}: {value_voll:f}".format(
                    ...
                    print(
                        "StarDist, {now}   thresh: {prob_thresh:f}   {measure}: {value_star:f}".format(
                            now=datetime.datetime.now().strftime("%H:%M:%S"),
                            prob_thresh=prob_thresh,
                            measure=measure,
                            value=value_star,
                    ...
                    progress.set_postfix_str(
                        "VollSeg-StarDist, {prob_thresh:.3f} -> {value_voll:.3f}, {value_star:.3f}".format(
                            prob_thresh=prob_thresh, value_voll=value_voll, value_star=value_star
                        )
                    )
