...
class Augmentation2D(object):
    ...
    def build(self, data=None, label=None, batch_size=None):
        """
        Arguments:
    ...
    def _zoom_data(self, data, parse_dict):
        ...
        temp = [
            [math.floor((i - j) / 2), math.ceil((i - j) / 2)] for i, j in zip(self.data_shape[1:], zoomed.shape[1:])
        ]

        cast_zoomed = np.zeros(data_shape)
        cast_zoomed[
            temp[0][0] : self.data_shape[1] - temp[0][1], temp[1][0] : self.data_shape[2] - temp[1][1]
        ] = zoomed
