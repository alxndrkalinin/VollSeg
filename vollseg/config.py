...
class Config(BaseConfig):
    ...
    def __init__(
        self, axes="YX", n_channel_in=1, n_channel_out=1, probabilistic=False, allow_new_parameters=False, **kwargs
    ):
        """See class docstring."""
    ...
    def is_valid(self, return_invalid=False):
        ...
        def _is_int(v, low=None, high=None):
            return isinstance(v, int) and (True if low is None else low <= v) and (True if high is None else v <= high)
        ...
        ok["unet_residual"] = isinstance(self.unet_residual, bool) and (
        ...
        ok["train_loss"] = (self.probabilistic and self.train_loss == "laplace") or (
