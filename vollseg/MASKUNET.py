...
class MASKUNET(BaseModel):
    """Standard MASKUNET network for image segmentation based on CARE network.
    ...
    def train(self, X, Y, validation_data, epochs=None, steps_per_epoch=None):
        ...
        if (
            ...
            and not IS_TF_1
            and not any(isinstance(cb, CARETensorBoardImage) for cb in self.callbacks)
        ):
            self.callbacks.append(
    ...
    def predict_probabilistic(
        self, img, axes, normalizer=PercentileNormalizer(), resizer=PadAndCropResizer(), n_tiles=None
    ):
    ...
    def _predict_mean_and_scale(self, img, axes, normalizer, resizer, n_tiles=None):
        ...
        def _total_n_tiles(n_tiles):
            n_block_overlaps = [
                int(np.ceil(1.0 * tile_overlap / block_size))
                for tile_overlap, block_size in zip(net_axes_in_overlaps, net_axes_in_div_by)
            ]
        ...
        (
            ...
            or _raise(ValueError("entry of n_tiles > 1 only allowed for axes '%s'" % tiling_axes))
        )
        ...
        while not done:
            try:
                ...
                x = predict_tiled(
                    ...
                    x,
                    axes_in=net_axes_in,
                    axes_out=net_axes_out,
                    n_tiles=n_tiles,
                    block_sizes=net_axes_in_div_by,
                    tile_overlaps=net_axes_in_overlaps,
                    pbar=progress,
                )
