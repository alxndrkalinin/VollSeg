...
def register_aliases(cls, key, *names):
    ...
    for name in names:
        aliases.get(name, key) == key or warn(
            "alias '%s' was previously registered with model '%s' for '%s'" % (name, aliases[name], cls.__name__)
        )
        aliases[name] = key
...
def get_model_folder(cls, key_or_alias):
    ...
    path = Path(get_file(fname=key + ".zip", origin=m["url"], file_hash=m["hash"], cache_subdir=target, extract=True))
