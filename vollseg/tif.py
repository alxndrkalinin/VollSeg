...
def mantissa(value):
    ...
    n_digits = len(digits)
    dividend = int(sum(v * (10 ** (n_digits - 1 - i)) for i, v in enumerate(digits)) * (1 if sign == 0 else -1))
    divisor = int(10**-exp)
...
def write_tif(filename, obj):
    ...
    imwrite(filename, obj.T, imagej=True, resolution=(vx, vy), metadata=metadata)
...
def pylibtiff_write_file(
    tif, filename, compression="none", strip_size=2**13, planar_config=1, validate=False, verbose=None, info={}
):
    ...
    if verbose is None:
        ...
        compression_map = dict(packbits=32773, none=1, lzw=5, jpeg=6, ccitt1d=2, group3fax=3, group4fax=4)
        compress_map = dict(none=lambda data: data, lzw=tif_lzw.encode)
        decompress_map = dict(none=lambda data, bytes: data, lzw=tif_lzw.decode)
        ...
        for i, image in enumerate(tif.data):
            ...
            d.update(info)
            if i == 0:
                d.update(dict(ImageDescription=tif.description, Software="http://code.google.com/p/pylibtiff/"))
        ...
        def tif_write(tif, offset, data, tifs=[]):
            ...
            if end > tif.size:
                ...
                new_tif = np.ndarray.__new__(np.memmap, (tif._mmap.size(),), dtype=tif.dtype, buffer=tif._mmap)
                new_tif._parent = tif
        ...
        if compressed_data_size != image_data_size:
            ...
            if verbose:
                sys.stdout.write(
                    ...
                    % (bytes2str(total_size + sdiff), bytes2str(total_size), compression)
                )
                sys.stdout.flush()
        del tif  # flushing
        return compression
